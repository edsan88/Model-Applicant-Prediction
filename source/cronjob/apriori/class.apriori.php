...
class Apriori {
    ...
    public function printFreqItemsets($position_name,$link)
    {
       ...
       foreach($this->freqItmsts as $k => $v)
          {
			...
			$this->SaveItemSet($this->minSup,$this->minConf,$position_name,$tmp,$this->allsups[$tmp1],$link);	
          }
    }   
    ...
    public function printAssociationRules($position_name,$link)
    {
             ...
             foreach($arr as $b => $conf)
                { 
                   if(strpos($a, 'ACTUAL WORK YEARS') !== false){
						...
						$this->SaveAssociationRulesInDB($this->minSup,$this->minConf,$position_name,$a,$b,$conf,1,$link);
				   }else{
						echo "$this->minSup , $this->minConf , $position_name : $a => $b = $conf%<br />";  
						$this->SaveAssociationRulesInDB($this->minSup,$this->minConf,$position_name,$a,$b,$conf,0,$link);
				   }
				   
                }
          }
    }
	public function SaveAssociationRulesInDB($minsup,$mincon,$position_name,$rule1,$rule2,$rule_confidence,$status,$link){
		$s = "SELECT * FROM retention.employee_association_rules 
			WHERE employee_association_rules.support = '".$minsup."' 
			AND employee_association_rules.confidence = '".$mincon."' 
			AND employee_association_rules.position_name = '".$position_name."' 
			AND employee_association_rules.rule1 = '".$rule1."' 
			AND employee_association_rules.rule2 = '".$rule2."' 
			AND employee_association_rules.rule_confidence = '".$rule_confidence."' 
			AND employee_association_rules.status = '".$status."'";
		$d = mysqli_query($link,$s);
		if(mysqli_num_rows($d)>0){
			//$sql = "UPDATE retention.employee_association_rules";
		}else{
			$sql = "INSERT INTO retention.employee_association_rules 
					VALUES(null,'".$minsup."','".$mincon."','".$position_name."','".$rule1."','".$rule2."','".$rule_confidence."','".$status."')";
			mysqli_query($link,$sql);		
		}
	}
	public function SaveItemSet($minsup,$mincon,$position_name,$itemset,$count,$link){
		$s = "SELECT * FROM retention.itemset 
			WHERE itemset.support = '".$minsup."'
			AND itemset.confidence = '".$mincon."' 
			AND itemset.position = '".$position_name."' 
			AND itemset.itemset = '".$itemset."' 
			AND itemset.count = '".$count."'";
		$d = mysqli_query($link,$s);
		if(mysqli_num_rows($d)>0){
			//$sql = "UPDATE retention.employee_association_rules";
		}else{
			$sql = "INSERT INTO retention.itemset 
					VALUES(null,'".$minsup."','".$mincon."','".$position_name."','".$itemset."','".$count."')";
			mysqli_query($link,$sql);		
		}
	}
