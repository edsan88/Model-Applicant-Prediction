...
function SelectWordFrequencyByUserResponseID($user_overall_response_id,$link){
	$sql = "SELECT * FROM retention.extract_pos_word
			WHERE extract_pos_word.user_overall_response_id = '".$user_overall_response_id."'";
	$data = mysqli_query($link,$sql);
	if(mysqli_num_rows($data)>0){
		$counter = 1;
		while($row = mysqli_fetch_array($data)){
			//$row['word_frequency_list_id'];
			$strings = array($counter=>$row['phrase']);
			//require_once __DIR__ . '/../autoload.php';
			...
			foreach ($strings as $string) {
				...
				foreach($scores as $x => $x_value) {
					...
					UpdateWordFrequencyListSentiments($row['extract_pos_word'],$x,$x_value,$link);
				}
			}
			$counter ++;
		}
...
function UpdateWordFrequencyListSentiments($id,$field,$field_value,$link){
	if($field == 'pos'){
		$sql = "UPDATE retention.extract_pos_word
				SET extract_pos_word.sentiment_pos = '".$field_value."'
				WHERE extract_pos_word.extract_pos_word = '".$id."'";
	}else if($field == 'neu'){
		$sql = "UPDATE retention.extract_pos_word
				SET extract_pos_word.sentiment_neu = '".$field_value."'
				WHERE extract_pos_word.extract_pos_word = '".$id."'";
	}else if($field == 'neg'){
		$sql = "UPDATE retention.extract_pos_word
				SET extract_pos_word.sentiment_neg = '".$field_value."'
				WHERE extract_pos_word.extract_pos_word = '".$id."'";
	}		
