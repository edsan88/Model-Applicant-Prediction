...
if(mysqli_num_rows($data)>0){
	...
	while($row = mysqli_fetch_array($data)){
		$strings = array($counter=>$row['consolidated_response']);
		//require_once __DIR__ . '/../autoload.php';
		...
		foreach ($strings as $string) {
			...
			foreach($scores as $x => $x_value) {
				...
				UpdateUserOverallResponseSentiments($row['user_overall_response_id'],$x,$x_value,$link);
			}
		}
		$counter ++;
	}
...
function UpdateUserOverallResponseSentiments($id,$field,$field_value,$link){
	if($field == 'pos'){
		$sql = "UPDATE retention.user_overall_response 
			SET user_overall_response.sentiment_pos = '".$field_value."' 
			WHERE user_overall_response.user_overall_response_id = '".$id."'";
	}else if($field == 'neu'){
		$sql = "UPDATE retention.user_overall_response 
			SET user_overall_response.sentiment_neu = '".$field_value."' 
			WHERE user_overall_response.user_overall_response_id = '".$id."'";
	}else if($field == 'neg'){
		$sql = "UPDATE retention.user_overall_response 
			SET user_overall_response.sentiment_neg = '".$field_value."' 
			WHERE user_overall_response.user_overall_response_id = '".$id."'";
	}		
